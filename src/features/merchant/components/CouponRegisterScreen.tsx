  ...
  if (monsters.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 px-5 text-center">
        ...
        <p className="text-sm text-gray-400">먼저 몬스터 등록 탭에서 몬스터를 만들어주세요</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-5 p-5">
      <div>
        ...
        <p className="text-xs text-gray-400 mt-0.5">몬스터를 선택하고 쿠폰 1개를 등록해요</p>
      </div>

      <p className="text-xs text-gray-500 bg-blue-50 rounded-xl px-4 py-3">
        💡 유저가 해당 몬스터를 잡으면 등록된 쿠폰 중 하나를 보상으로 받아요
      </p>
          ...
          {monsters.map((m) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">{m.attribute} 속성</p>
              </div>
      ...
      {showPopup && selected && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-t-3xl">
            ...
            <p className="text-xs text-center text-gray-400 mt-3 px-5">
              유저가 {selected.name}을 잡으면 이 쿠폰을 보상으로 받아요
            </p>
